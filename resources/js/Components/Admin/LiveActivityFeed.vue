...
defineEmits(['update:category', 'update:search', 'toggle-pause', 'reveal-new', 'load-more', 'inspect', 'action']);
...
function metadataBadges(event) {
    ...
    return [meta.amount, meta.category, meta.state, meta.local_government, meta.rating ? `${meta.rating} stars` : null]
        .filter(Boolean);
}
