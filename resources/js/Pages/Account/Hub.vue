    ...
    <AppShell>
        ...
        <div class="mx-auto w-full max-w-4xl space-y-8">
                    ...
                    <div class="flex flex-col gap-5 sm:flex-row sm:items-start">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="mt-3 flex flex-wrap gap-2 text-sm font-semibold text-slate-600">
                                <span
                                    v-if="proMembership?.subscription?.is_pro"
                                    class="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-amber-400 to-amber-500 px-3 py-1 font-black uppercase tracking-wide text-amber-950 ring-1 ring-amber-600/30"
                                >
                                    Pro member
                                </span>
                            ...
                            <p
                                v-if="user.role_slug === 'freelancer' && (publicReviewsUrl || publicPortfoliosUrl || proMembership)"
                                class="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-sm font-semibold"
                            >
                                <Link
                                    v-if="proMembership"
                                    :href="route('freelancer.pro.index')"
                                    class="text-amber-800 underline-offset-4 hover:underline"
                                >
                                    {{ proMembership.subscription.is_pro ? 'Manage Pro membership' : 'Upgrade to Pro' }}
                                </Link>
                                <Link
            ...
            <div v-show="localTab === 'overview'" class="space-y-8">
                ...
                >
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            ...
                            <p
                                v-if="verificationEngine.waiting_for_account_age && verificationEngine.next_level_label"
                                class="mt-1 text-xs font-semibold text-amber-900"
                            >
                                You remain at {{ verificationEngine.current_label || ('L' + verificationEngine.current_level) }} until you reach
                                {{ verificationEngine.next_level_label }}.
                            </p>
                ...
                <section
                    v-if="user.role_slug === 'freelancer' && proMembership"
                    class="overflow-hidden rounded-[1.75rem] border border-amber-200/80 bg-gradient-to-br from-amber-50/90 via-white to-primary-50/60 p-6 shadow-sm ring-1 ring-amber-100 sm:p-8"
                >
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            <p class="text-xs font-black uppercase tracking-[0.2em] text-amber-800">Freelancer Pro</p>
                            <h2 class="font-display mt-2 text-lg font-bold text-slate-900">
                                {{ proMembership.subscription.is_pro ? 'Your premium benefits are active' : 'Win more work with Pro' }}
                            </h2>
                            <p class="mt-2 max-w-2xl text-sm font-medium leading-relaxed text-slate-600">
                                <template v-if="proMembership.subscription.is_pro">
                                    Unlimited proposals, priority visibility, faster verification reviews, and priority support — your verification tier and job value limits stay the same.
                                </template>
                                <template v-else>
                                    Pro adds convenience and visibility on top of your trust tier. It does not bypass verification, account age, or job value limits.
                                </template>
                            </p>
                            <div v-if="!proMembership.subscription.is_pro" class="mt-4">
                                <p class="text-xs font-black uppercase tracking-wide text-slate-500">This month</p>
                                <p class="mt-1 text-sm font-semibold text-slate-800">
                                    {{ proMembership.quota.used }} of {{ proMembership.quota.limit }} proposals used
                                </p>
                                <div class="mt-2 h-2 max-w-xs overflow-hidden rounded-full bg-white ring-1 ring-slate-200">
                                    <div
                                        class="h-full rounded-full bg-primary-600 transition-all"
                                        :style="{ width: `${Math.min(100, proMembership.quota.percent_used)}%` }"
                                    />
                                </div>
                            </div>
                            <p v-else class="mt-3 text-sm font-semibold text-emerald-800">
                                Renews {{ formatProRenewal(proMembership.subscription.renewal_date) }}
                                <span v-if="proMembership.subscription.billing_cycle_label">
                                    · {{ proMembership.subscription.billing_cycle_label }}
                                </span>
                            </p>
                        </div>
                        <Link
                            :href="route('freelancer.pro.index')"
                            class="inline-flex shrink-0 items-center justify-center rounded-full px-5 py-2.5 text-xs font-black uppercase tracking-wide text-white shadow-sm"
                            :class="proMembership.subscription.is_pro ? 'bg-slate-800 hover:bg-slate-900' : 'bg-amber-600 hover:bg-amber-700'"
                        >
                            {{ proMembership.subscription.is_pro ? 'Manage Pro' : 'View plans & upgrade' }}
                        </Link>
                    </div>
                </section>
                ...
                >
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm font-medium text-slate-600">
                                Gateway funding is coming soon — you can still open structured disputes and follow deadlines.
                            </p>
                        </div>
                        <Link
                            :href="commerce_hub.disputes_index_url"
                            class="inline-flex shrink-0 items-center justify-center rounded-full bg-emerald-700 px-5 py-2.5 text-xs font-black uppercase tracking-wide text-white shadow-sm hover:bg-emerald-800"
                        >
                            Disputes ({{ commerce_hub.open_disputes_count }})
                        </Link>
                    </div>
                    <p v-if="Number(commerce_hub.pending_funding_quests_count) > 0" class="mt-4 text-sm font-semibold text-amber-900">
                        You have {{ commerce_hub.pending_funding_quests_count }} quest(s) waiting for escrow funding — open each quest to launch checkout when live.
                    </p>
                </section>
...
const props = defineProps({
    ...
    commerce_hub: { type: Object, default: null },
    proMembership: { type: Object, default: null },
});
