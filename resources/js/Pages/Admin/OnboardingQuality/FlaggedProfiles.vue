<template>
    <AdminShell title="Flagged profiles" subtitle="Accounts flagged for monitoring during onboarding quality review.">
        <div class="mb-4">
            <Link :href="route('admin.onboarding-quality.index')" class="text-xs font-black uppercase text-primary-700">← Onboarding quality control</Link>
        </div>
        ...
        <OperationsSlideOver :open="slideOpen" :title="detail?.user?.name || 'Flagged profile'" eyebrow="Monitoring" @close="slideOpen = false">
            <div v-if="detail" class="space-y-3 text-sm">
                ...
                <Link :href="route('admin.onboarding-quality.index')" class="inline-block text-xs font-black uppercase text-primary-700">Open in onboarding QC →</Link>
            </div>
        </OperationsSlideOver>
    </AdminShell>
</template>
...
async function reload() {
    loading.value = true;
    try {
        const { data } = await window.axios.get(route('admin.api.onboarding-quality.flagged'), {
